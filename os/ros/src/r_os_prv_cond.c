...
****************************************************************************
PROJECT : eeOS2
FILE    : $Id: r_os_prv_cond.c 11006 2016-11-07 16:32:44Z michael.golczewski $
                            ...
                            R_OS_ThreadMutex_t *Mutex)
{
    ...
    Cond->Mutex = Mutex;
    R_OS_Prv_ThreadMutexUnlock(Mutex);
    R_OS_Prv_ThreadWait(R_OS_TSTATE_WAITING, Cond);
    R_OS_Prv_ThreadMutexLock(Mutex);
    rc = 0;
    return rc;
}
...
int32_t R_OS_Prv_ThreadCondSignal(R_OS_ThreadCond_t *Cond)
{
    ...
    rc = _CheckCondEvent(Cond);
    if (rc != 0)
    {
        R_OS_Prv_ThreadMutexUnlock(Cond->Mutex);
        R_OS_Prv_ThreadSchedule();
        R_OS_Prv_ThreadMutexLock(Cond->Mutex);
    }
    rc = 0;
    return rc;
}
